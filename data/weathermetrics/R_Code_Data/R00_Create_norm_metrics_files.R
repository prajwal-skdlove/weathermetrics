...
fn_create_norm_metrics_files <- function(in_yrs,in_station){
if(TRUE==FALSE){
  in_yrs <- lst_years
...
lst_metrics <- fn_aggregate_metrics(wd,fn,dt_metrics_year_norm)
...
# OH, PA, VT, NH, ME, MA, RI, CT, MT, ND, SD, NE, KS, FL (station 44 has problem) CA, TX NY, NJ

vc_stations <- dt_stations[US_State %in% c("VA","NC","DE","MD"),STATION]

for(istn in vc_stations){
    check <- fn_create_norm_metrics_files(lst_years,istn)
}
