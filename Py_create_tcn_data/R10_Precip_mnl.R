...
fn_pqt_one_model <- function(fpn,woe_cutoff){
  ...
  Sys.time()
  vc_vars <- 1:432 #three days of 6 measures at 1 hour intervals
...
Sys.time()
fpn1 <- dt_files[3,2]
fpn <- paste("C:\\Users\\jhugh\\Documents\\Py_S4\\NCEI_parquet_files\\",dt_files[3,1],sep="")
woe_cutoff <- 0.1
...
#output_file <- "C:\\Users\\jhugh\\Documents\\Py_S4\\NCEI_parquet_files\\my_data.parquet"
#write_parquet(dt_files, sink = output_file)
#jj <- read_parquet(output_file)
...
table(jj$tgt_bin,cut(as.numeric(unlist(jj[,'5'])),breaks=jjbk))
